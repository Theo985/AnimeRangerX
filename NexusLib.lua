...
local function InitNotifications(screenGui)
    NotificationHolder = Utility.Frame({
...
local function CreatePlayerProfile(parent)
    ...
    Utility.Label({ Text = LocalPlayer.Name, Color = Theme.TextPrimary, Font = Theme.FontBold, Size = 14, FrameSize = UDim2.new(1, -80, 0, 22), Position = UDim2.new(0, 72, 0, 12), Parent = profileCard })
    local statusDot = Utility.Frame({ Name = "StatusDot", Color = Theme.NotifySuccess, Size = UDim2.new(0, 8, 0, 8), Position = UDim2.new(0, 72, 0, 38), Parent = profileCard })
    Utility.Corner(statusDot, UDim.new(0, 4))
    Utility.Label({ Text = "Freemium", Color = Theme.NotifyWarning, Size = 11, FrameSize = UDim2.new(1, -80, 0, 18), Position = UDim2.new(0, 86, 0, 36), Parent = profileCard })
...
local function CreateRGBPickerContent(parent, currentColor, callback)
    ...
    local function makeSlider(y, labelColor, labelText, initialVal)
        ...
        Utility.Corner(fill)
        local knob = Utility.Button({ Text = "", BgColor = Color3.fromRGB(255,255,255), FrameSize = UDim2.new(0, 12, 0, 12), Position = UDim2.new(initialVal, -6, 0.5, -6), Parent = track })
        Utility.Corner(knob, UDim.new(0, 6))
        return { lbl = lbl, track = track, fill = fill, knob = knob, value = initialVal }
    end
    ...
    local function updateSlider(slider, val, channel)
        slider.value = val
        slider.fill.Size = UDim2.new(val, 0, 1, 0)
        slider.knob.Position = UDim2.new(val, -6, 0.5, -6)
        slider.lbl.Text = ({ "Red", "Green", "Blue" })[channel] .. ": " .. math.floor(val * 255)
    ...
    local function startDrag(slider, channel)
        return function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                dragging[channel] = true
                local pos = math.clamp((Mouse.X - slider.track.AbsolutePosition.X) / slider.track.AbsoluteSize.X, 0, 1)
                updateSlider(slider, pos, channel)
            end
        end
    end

    red.track.InputBegan:Connect(startDrag(red, 1))
    green.track.InputBegan:Connect(startDrag(green, 2))
    blue.track.InputBegan:Connect(startDrag(blue, 3))

    UserInputService.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            if dragging[1] then
                local pos = math.clamp((Mouse.X - red.track.AbsolutePosition.X) / red.track.AbsoluteSize.X, 0, 1)
                updateSlider(red, pos, 1)
            elseif dragging[2] then
                local pos = math.clamp((Mouse.X - green.track.AbsolutePosition.X) / green.track.AbsoluteSize.X, 0, 1)
                updateSlider(green, pos, 2)
            elseif dragging[3] then
                local pos = math.clamp((Mouse.X - blue.track.AbsolutePosition.X) / blue.track.AbsoluteSize.X, 0, 1)
                updateSlider(blue, pos, 3)
            end
        end
    end)
...
function NexusLib:CreateWindow(opts)
    ...
    if configName and type(configName) == "string" then
        local success, res = pcall(Configs.Load, Configs, configName)
        if success and res then savedConfig = res end
    end
    ...
    local minBtn = Utility.Button({ Name = "MinBtn", Text = "−", Color = Theme.TextSecondary, BgColor = Color3.fromRGB(28,28,38), FrameSize = UDim2.new(0, 34, 0, 34), Position = UDim2.new(0, 0, 0, 0), Parent = btnGroup })
    minBtn.TextSize = 22
    Utility.Corner(minBtn, UDim.new(0, 8))
    local minGlow = Utility.Frame({ Name = "Glow", Color = Theme.Accent, Size = UDim2.new(1,0,1,0), Parent = minBtn })
    ...
    local closeBtn = Utility.Button({ Name = "CloseBtn", Text = "✕", Color = Theme.TextSecondary, BgColor = Color3.fromRGB(28,28,38), FrameSize = UDim2.new(0, 34, 0, 34), Position = UDim2.new(0, 38, 0, 0), Parent = btnGroup })
    closeBtn.TextSize = 18
    Utility.Corner(closeBtn, UDim.new(0, 8))
    local closeGlow = Utility.Frame({ Name = "Glow", Color = Color3.fromRGB(220,70,70), Size = UDim2.new(1,0,1,0), Parent = closeBtn })
    ...
    local tabBarDivider = Utility.Frame({ Name = "Divider", Color = Theme.Border, Size = UDim2.new(0, 1, 1, 0), Position = UDim2.new(1, 0, 0, 0), Parent = tabBar })
    ...
    function WindowObj:AddTab(name, icon)
        ...
        -- Helper pour wrapper avec animation
        ...
        function tabData:AddSection(name)
            local sectionLabel = Utility.Label({ Text = name, Color = Theme.Accent, Font = Theme.FontBold, Size = 12, FrameSize = UDim2.new(1, 0, 0, 26), Parent = scroll })
        ...
        function tabData:AddToggle(opts)
            ...
            local trackBg = Utility.Frame({ Name = "TrackBg", Color = default and Theme.ToggleOn or Theme.ToggleOff, Size = UDim2.new(0, 42, 0, 24), Position = UDim2.new(1, -54, 0.5, -12), Parent = wrap })
            Utility.Corner(trackBg, UDim.new(0, 12))
            local knob = Utility.Frame({ Name = "Knob", Color = Theme.ToggleKnob, Size = UDim2.new(0, 18, 0, 18), Position = default and UDim2.new(0, 22, 0.5, -9) or UDim2.new(0, 3, 0.5, -9), Parent = trackBg })
        ...
        function tabData:AddSlider(opts)
            ...
            local function updateFromMouse()
                local pos = math.clamp((Mouse.X - trackBg.AbsolutePosition.X) / trackBg.AbsoluteSize.X, 0, 1)
        ...
        function tabData:AddKeybind(opts)
            ...
            UserInputService.InputBegan:Connect(function(input, gameProcessed)
                if listening and input.UserInputType == Enum.UserInputType.Keyboard then
                    ...
                    if saveKey and configName then
                        local cfg = Configs.Load(configName)
                        cfg[saveKey] = input.KeyCode.Name
                        Configs.Save(configName, cfg)
                    end
        ...
        function tabData:AddColorPicker(opts)
            ...
            swatchScroll.CanvasSize = UDim2.new(0, 0, 0, 28)
            swatchScroll.AutomaticCanvasSize = Enum.AutomaticSize.X
            swatchScroll.Parent = wrap
            ...
            swatchList.Parent = swatchScroll
        ...
        function tabData:AddSeparator()
            local sep = Utility.Frame({ Name = "Separator", Color = Theme.Border, Size = UDim2.new(1, 0, 0, 1), Parent = scroll })
        end
    ...
    OnThemeChange(function(newTheme)
        ...
        if playerProfile then
            ...
            local avatar = playerProfile:FindFirstChild("AvatarImage")
            if avatar then avatar.BackgroundColor3 = newTheme.Accent end
            local name = playerProfile:FindFirstChild("Label")
            if name then name.TextColor3 = newTheme.TextPrimary end
        end
        saveConfig()
    end)
